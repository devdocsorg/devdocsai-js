...
export interface SearchViewProps {
  ...
  projectKey: string;
  searchOptions?: DevDocsAIOptions['search'];
}
...
const searchInputName = 'devdocsai-search';
...
export function SearchView(props: SearchViewProps): ReactElement {
  ...
  const searchOptions = useDefaults(
    { ...props.searchOptions },
    DEFAULT_DEVDOCSAI_OPTIONS.search,
  );
  ...
  return (
    <div className="DevDocsAISearchView">
...
function SearchResultsContainer(
  ...
  return (
    <div className="DevDocsAISearchResultsContainer">
      {state === 'done' &&
        searchResults.length === 0 &&
        searchQuery.trim().length > 0 && (
          <div className="DevDocsAINoSearchResults">
            <p>
      ...
      {searchResults.length > 0 && (
        <BaseDevDocsAI.SearchResults
          searchResults={searchResults}
          className="DevDocsAISearchResults"
          SearchResultComponent={({ index, ...rest }) => {
            const id = `devdocsai-result-${index}`;
            return (
              <SearchResult
                {...rest}
                id={`devdocsai-result-${index}`}
                searchQuery={searchQuery}
                onMouseMove={() => {
